...
'use strict';
   ...
   * @type {angular.Module}
   */
// var app = angular.module('MyApp',['ngMaterial', 'ngMessages', 'ngRoute', 'ngNewRouter', 'ngCookies']);
...
//     { path: '/menu', name: 'MenuPage', templateUrl: 'menu.html', controller: 'MenuCtrl', controllerAs: 'menu', useAsDefault: true }
//   ]);
// });
...
// KassaCtrl.$inject = ['$rootScope', '$scope', '$log'];

// function KassaCtrl ($rootScope, $scope, $log) {
//   var self = this;
...
// LoginCtrl.$inject = ['$rootScope', '$scope', '$location', '$log', 'AuthenticationService', 'FlashService'];
// function LoginCtrl('$rootScope', $location, $scope, $log, AuthenticationService, FlashService) {
LoginCtrl.$inject = ['$rootScope', '$scope', '$location', '$log'];

function LoginCtrl($rootScope, $scope, $location, $log) {
  var self = this;
  ...
  function login() {
    ...
    if (self.password === '1234509876') {
      $location.path('/menu');
    } else {}
    // $scope.loginForm.password.$error = true;

    // $log.debug($scope.loginForm.password);
...
// MenuCtrl.$inject = ['$rootScope', '$scope', '$log', '$route', '$routeParams', '$location'];
// function MenuCtrl($rootScope, $scope, $log, $route, $routeParams, $location) {
MenuCtrl.$inject = ['$rootScope', '$scope', '$log', '$route', '$location'];

function MenuCtrl($rootScope, $scope, $log, $route, $location) {
  var self = this;

  self.isDisabled = false;
  self.$route = $route;
  self.Go = Go;
  self.name = $route.current.name;
  // self.name = $route.current.scope.name;
  ...
  function Go(url) {
    $log.info('MenuCtrl --> Go:' + url + '(' + self.name + ')');
    $location.path(url);
    // self.$router.navigate(['CrisisList', {id: crisisId}]);
    // $scope.$router.navigate(['KassaPage']);
  }
}
/* jshint undef: true, unused: true */
/* global angular */
...
ToolbarCtrl.$inject = ['$rootScope', '$scope', '$log', '$route', '$location'];

function ToolbarCtrl($rootScope, $scope, $log, $route, $location) {
  var self = this;
  ...
  // self.Go = Go;
  self.name = $route.current.name;
  $log.info('Page:' + self.name);
  if (self.name === 'KassaPage') {
    $log.info('set order code show');
    self.isKassa = true;
  }
